...
// 对linux日志进行c++封装 ，便于使用ini配置文件来配置日志参数
// 使用方法： 配置文件如data.ini格式，在代码中的使用方法：
/***********************************************************
  1, 0 != xlog::inst().init( config_file );
  2, xlog::inst().info(...);

    配置文件支持动态加载，其在配置文件中的格式如下：
    [log]
    ident = hello
    options = cons,nodelay,pid
    facility = local3

    在代码中调用info() ...等方法时，但不想在运行中输出，可在下面配置
    中选择输出：
